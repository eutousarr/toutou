...
class Fournisseur(db.Model):
    ...
    bons = db.relationship('Bon', backref='fournisseur')
class Bon(db.Model):
    
    __tablename__ = 'bon'
    
    id = db.Column(db.Integer, primary_key=True)
    annee = db.Column(db.String, nullable=False)
    mois = db.Column(db.String, nullable=False)
    ...
    def __init__(self, *args, **kwargs):
        if not 'slug' in kwargs:
            kwargs['slug'] = kwargs.get(['annee'])+'-'+kwargs.get(['mois'])
